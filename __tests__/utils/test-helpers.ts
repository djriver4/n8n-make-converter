...
export function loadFixture(
  ...
  options: {
    ...
    logWarnings?: boolean;
  } = { useFallback: true, validateStructure: true, logWarnings: true }
): WorkflowWithSource {
  ...
  try {
    ...
    const fixtureData: WorkflowWithSource = JSON.parse(fileContents);
    ...
    if (options.useFallback) {
      ...
      const fallbackData: WorkflowWithSource = platform === 'n8n' ? 
        { ...DEFAULT_N8N_WORKFLOW } : 
        { ...DEFAULT_MAKE_WORKFLOW };
...
export function compareWorkflows(
  ...
  if (Array.isArray(actual) && Array.isArray(expected)) {
    ...
    for (let i = 0; i < minLength; i++) {
      const { matches: elementMatches, differences: elementDifferences } = compareWorkflows(
        actual[i], 
        expected[i], 
        { 
          ignoreFields,
  ...
  if (typeof actual === 'object' && typeof expected === 'object') {
    ...
    for (const key of Object.keys(expected)) {
      ...
      const { matches: valueMatches, differences: valueDifferences } = compareWorkflows(
        actual[key], 
        expected[key], 
        { 
          ignoreFields,
...
export function toMatchWorkflowStructure(received: any, expected: any, options: any = {}) {
  ...
  let structureType: 'n8n' | 'make' | 'unknown' = 'unknown';
  ...
  const structureMatches = 
    (structureType === 'n8n' && 'nodes' in received) || 
