 ...
 */
export const specializedMappings: Record<string, NodeMapping> = {
  /**
  ...
  'httpRequest': {
    ...
    parameterMappings: [
      ...
      },
      {
        ...
        transform: (value: ParameterValue) => {
          ...
          return authMap[value as string] || 'none';
        }
      },
      {
        sourcePath: 'headers',
      ...
      },
      {
        ...
        transform: (value: ParameterValue) => value === 'json' || value === 'xml'
      }
    ],
    ...
    parameterMappings: [
      {
        ...
        transform: (value: ParameterValue) => {
          ...
          return operationMap[value as string] || value;
        }
      },
      {
    ...
    parameterMappings: [
      {
        ...
        transform: (value: ParameterValue) => {
          if (!value || typeof value !== 'object') return { mode: 'and', conditions: [] };
          
          // Transform n8n filter conditions to Make.com format
          const makeConditions: any[] = [];
  ...
  'if': {
    ...
    customTransform: (sourceNode: N8nNode, context: ExpressionContext): Partial<MakeModule> => {
      ...
      if (sourceNode.parameters && sourceNode.parameters.conditions) {
        const conditions = sourceNode.parameters.conditions as Record<string, any>;
        let conditionField1 = "";
    ...
    parameterMappings: [
      {
        ...
        transform: (value: ParameterValue) => {
          ...
          return modeMap[value as string] || 'append';
        }
      },
      {
 ...
 */
function mapFilterOperation(operation: string): string {
  const operationMap: Record<string, string> = {
    'contains': 'contains',
 ...
 */
export function registerSpecializedMappings(mappingLoader: any): void {
  for (const [key, mapping] of Object.entries(specializedMappings)) {
    mappingLoader.registerMapping(key, mapping);
  }
}

export default specializedMappings; 