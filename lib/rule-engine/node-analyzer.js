...
class NodeAnalyzer {
    ...
    analyzeWorkflow(workflow) {
        ...
        if (workflow.nodes && Array.isArray(workflow.nodes)) {
            ...
            return workflow.nodes.map((node) => this.analyzeNode(node, workflow));
        }
        ...
        if (workflow.flow && Array.isArray(workflow.flow)) {
            ...
            return workflow.flow.map((module) => {
    ...
    addDefaultRules() {
        ...
        this.addRule({
            ...
            check: (node) => {
                return (node.type && (node.type.includes("webhook") || node.type.includes("Webhook") || node.type.includes("trigger")));
            },
            suggestion: "Webhook configurations are platform-specific. Manual configuration may be required after conversion.",
            severity: "warning",
        });
