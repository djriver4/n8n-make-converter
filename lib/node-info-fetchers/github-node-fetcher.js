...
exports.fetchNodesFromGitHub = fetchNodesFromGitHub;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            for (const path of NODE_PATHS) {
                ...
                const response = yield fetchFunc(url, {
                    headers: {
                ...
                for (const file of files) {
                    if (file.type === 'dir' && !file.name.startsWith('.')) {
                        ...
                        const nodeInfo = yield fetchNodeInfo(file.name, path);
                        if (nodeInfo) {
 ...
 */
function fetchNodeInfo(nodeName, basePath) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!fetchFunc) {
            return null;
        }
        try {
            ...
            const response = yield fetchFunc(nodeJsonUrl);
            if (response.ok) {
                const nodeJson = yield response.json();
                return {
            ...
            const classResponse = yield fetchFunc(nodeClassUrl);
            if (classResponse.ok) {
                const nodeClass = yield classResponse.text();
